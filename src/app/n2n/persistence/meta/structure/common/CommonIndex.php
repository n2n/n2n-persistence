...
class CommonIndex extends IndexAdapter {
	...
	public static function createFromColumnNames(Table $table, string $name, string $type, array $columnNames) {
		$columns = [];
		foreach ($columnNames as $columnName) {
			$columns[] = $table->getColumnByName($columnName);
		}
		
		return new CommonIndex($table, $name, $type, $columns);
	}
}